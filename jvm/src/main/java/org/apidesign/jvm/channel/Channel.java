...
/**
 * Channel connects two {@link JVM} instances. A "channel" creates two (almost) identical instances of the
 * {@code Channel} on both sides of the "channel" - e.g. in each of the JVMs. The instances are
 * initialized with the same {@link Serde}, so they both understand the same messages and communicate
 * with each other.
 *
...
public final class Channel<Data extends Channel.Config> implements AutoCloseable {
   ...
   * mode"</em> via {@link #create(org.apidesign.ni.jvm.channel.JVM, java.lang.Class)} - such emulation is
   ...
   * registered for persistance via {@link Serde} annotation into the {@link
   * Serde pool associated with this JVM}. The result (which is of type {@code R}) also
   ...
   *     the other side and {@link Function#apply evaluated} there
   * @param <R> the type of result we expect the message to return
   * @return the value gets computed via {@link Function#apply} of {@code msg} in the other JVM and then it
