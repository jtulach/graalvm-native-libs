 ...
 */
package org.apidesign.jvm.persist.dsl;
...
/**
 * Processes the {@code Persistable} annotation. See its javadoc for proper
 * usage. A new subclass of {@code Persistance} class is generated per each
 * {@code Persistable} annotation.
 */
@SupportedAnnotationTypes({
    "org.apidesign.jvm.persist.Persistable",
    "org.apidesign.jvm.persist.Persistable.Group"
})
...
public final class PersistableProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        ...
        var Persistable = eu.getTypeElement("org.apidesign.jvm.persist.Persistable");
        var PersistableGroup = eu.getTypeElement("org.apidesign.jvm.persist.Persistable.Group");
        var Persistance = eu.getTypeElement("org.apidesign.jvm.persist.Persistance");
        if (Persistable == null || PersistableGroup == null || Persistance == null) {
            processingEnv.getMessager().printMessage(Kind.ERROR, "Cannot find @Persistable");
            return false;
        }
        try {
            for (var elem : roundEnv.getElementsAnnotatedWith(Persistable)) {
                var anno = getAnnotation(elem, Persistable);
                ok &= generatePersistance(elem, anno, Persistance);
            }
            ...
            for (var entry : registeredClasses.entrySet()) {
                try {
                    ...
                    try (var w = src.openWriter()) {
                        ...
                        w.append("import org.apidesign.jvm.persist.Persistance;\n");
    ...
    private boolean generatePersistance(Element orig, AnnotationMirror anno, Element Persistance) throws IOException {
        var eu = processingEnv.getElementUtils();
